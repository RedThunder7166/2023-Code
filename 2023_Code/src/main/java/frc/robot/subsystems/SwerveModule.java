...
public class SwerveModule extends SubsystemBase {
  ...
  // PID controller for velocity. DO NOT SET kD. It is redundant as setVoltage()
  ...
  public double getPosition() {
    double revolution = 2048;
    double gearRatio = (1 / 6.12);
    double metricCircumference = (Math.PI * 0.1016);
    double m_distanceTraveledMeters = (m_driveMotor.getSelectedSensorPosition() / revolution) * gearRatio
        * metricCircumference;

    return m_distanceTraveledMeters;
  }
  ...
  public void setDesiredState(SwerveModuleState desiredState) {
    ...
    double m_turningRadians = ((2 * Math.PI) / 360) * m_turnEncoder.getAbsolutePosition();
    ...
    SwerveModuleState state = SwerveModuleState.optimize(desiredState, new Rotation2d(m_turningRadians));
    ...
    final var turnOutput = m_turningPIDController.calculate(m_turningRadians, state.angle.getRadians())
